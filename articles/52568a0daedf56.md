...
ただし、`FQID`を手動で作成しているということは、内部構造に依存したコードなので変更に弱いです。また私見ですが、レイヤーを隠蔽するのが基本思想である以上、インポートされたスタイルのレイヤーを検索しなくなる変更が行われる可能性が考えられます。したがいまして、直接インポートされたスタイルのレイヤーのプロパティを変更するのは控えたほうが良いかもしれません。
